...
void quad_interp_init(QuadInterp *interp, const float p_0[2],
  ...
  interp->a = p_0[1] / (denom[0]) + p_1[1] / (denom[1]) + p_2[1] / (denom[2]);
...
void find_interpolation_points_linear(
  ...
  _find_interpolation_points_linear(t_tab_root->left, f_param_num, f, f_params,
                                    &params, t_tab_init, f_tab_init);
  ...
  *out_t_dyn = malloc(size * sizeof(float));
  ...
  tree_leaves_to_array(out_t_dyn, t_tab_root);
  tree_free(t_tab_root);
}

void _find_interpolation_points_linear(
    TreeNode *out_t_root, int f_param_num,
    const float (*f)(float, int, const float[2][f_param_num]),
  ...
  for (int i = 0; i < 3; i++) {
    f_test_interpolated[i] =
        f_tab[0] + (f_tab[2] - f_tab[0]) * extra_params->test_points[i];
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (abs_error > extra_params->abs_err_max) {
      ...
      _find_interpolation_points_linear(out_t_root->left, f_param_num, f,
                                        f_params, extra_params, t_tab_new,
                                        f_tab_new);
      ...
      _find_interpolation_points_linear(out_t_root->right, f_param_num, f,
                                        f_params, extra_params, t_tab_new,
                                        f_tab_new);
      break;
    }
...
float lerp_map(float t, const float (**t_x_map)[2], int map_size) {
  ...
  Lerp lerp_s;
  lerp_init(&lerp_s, (*t_x_map)[i - 1][1], (*t_x_map)[i][1],
            (*t_x_map)[i - 1][0], (*t_x_map)[i][0]);
