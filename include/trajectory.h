...
typedef struct {
  CubicCurve path;
  float (
      *p_tau_map)[2];  // table for p(tau) mapping function: p (path length from
                       // the start) value, tau (path curve parameter) value
  int p_tau_map_size;
  float t_phases[3];  // movement phase change moments: stop accelerating, start
                      // deccelerating, end trajectory
...
/*
Returns false and corrects value under v_f_ptr if didn't manage to reach
desired v_f (probably will never happen though). Follow with call to
trajectory_free when you're done using it.
...
/*
High level function to make time-spatial ((x, y)(t)) trajectory from spatial
path defined by single curve.

/*
Caluculates Trajectory t_phases and interpolators.
Returns false and corrects value under v_f_ptr if didn't manage to reach
desired v_f (probably will never happen though).
